...
const App = (props) => {
	...
	useEffect(() => {
		axios.get('http://localhost:5000/api/list-projects').then((resu) => {
			const arr = []
			resu.data.map((item) => arr.push(item.name))
	...
	const exportHtml = () => {
		emailEditorRef.current.editor.exportHtml((data) => {
			const { html } = data
			setHtml(html)
			return toast('Html Generated! Go to the bottom of the page', {
				type: 'info',
			})
		})
	}
	...
	const onLoad = (item) => {
		if (item) {
			setProject(item)
			axios
				.post('http://localhost:5000/api/get-project', {
					data: item,
				})
				.then((result) => {
					console.log(JSON.parse(result.data.template))
					emailEditorRef.current.editor.loadDesign(
						JSON.parse(result.data.template)
					)
	...
	const save = () => {
		emailEditorRef.current.editor.exportHtml((data) => {
			...
			if (project) {
				axios
					.post('http://localhost:5000/api/save-template', {
						data: JSON.stringify({
							name: project,
							design: design,
						}),
			...
			<Container style={{ marginTop: '20px' }}>
				{html && (
					<Form>
						...
						<TextArea
							value={html}
							style={{ minHeight: '600px', marginBottom: '60px' }}
						/>
					</Form>
				)}
			</Container>
